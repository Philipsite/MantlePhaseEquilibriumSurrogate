...
ax1 = Axis(fig[1,1],
           ...
           ylabel = L"\text{Molar fraction} [molmol^{-1}]",
...
for i in 1:22
    ...
    text!(ax1, i, 1.05, text=@sprintf("%0.2f%%", n/n_train * 100), rotation = π/2, align=(:left, :bottom), fontsize=10)
    ...
    if n > 0
        pos = repeat([i], n)
        side = repeat([:left], n)
        violin!(ax1, pos, vol_f, boundary = (0., 1.), color="lightgrey", orientation = :vertical, side=side, width = (1. * n / max_cts_val) + 0.2)
    end
...
fig
save(joinpath("05_figures_supplementary/fig_phases_in_training_dataset", "phase_modes_in_training_dataset.pdf"), fig)
